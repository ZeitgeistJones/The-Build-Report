...
export default async function Home() {
  ...
  // Slugs already hand-scored — don't auto-score these
  const handScoredSlugs = new Set(REPOS.map(r => r.githubSlug))

  // Find repos on GitHub that aren't in the hand-scored list
  const unscoredRepos = (stats?.allRepos ?? []).filter(
    r => !handScoredSlugs.has(r.name)
  )

  // Get auto-scores (Redis-cached, Claude inference on first sight)
  const autoScored = await getAutoScores(unscoredRepos)

  // Merge hand-scored + auto-scored, attach live GitHub activity to all
  const allRepos = [...REPOS, ...autoScored].map(r => ({
    ...r,
    ...
    <>
      ...
      <div id="how-we-score" style={{
        ...
        <div style={{ marginTop: '20px', fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.6 }}>
          ...
          and the clawdbotatg Chronicle. Hand scores are interpretive and updated manually.
          Auto-inferred scores (marked with ✦) are generated by Claude from repo metadata — treat as a starting point.
