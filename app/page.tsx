          ...
          {[
            {
              ...
              intro: 'What holder relevance grades: whether active work points toward real CLAWD holder value.',
              rows: [
              ...
              note: 'Each component is rated low (1), mid (2), or high (3). Score = (weighted sum ÷ 3) × 100. We score whether the mechanic exists and is live — not how much has burned. Infrastructure repos can be N/A at the repo level.',
            },
            {
              title: 'Builder integrity — all repos',
              intro: 'What builder integrity grades: whether a repo matches the stated vision, autonomous build standard, and walkaway test.',
              rows: [
              ...
              note: 'Repos are judged against the stated goal at the time they were built, not against later direction changes. CV burns are not CLAWD burns. Supply lock is not a burn.',
            },
            {
              title: 'Builder grade — GitHub signals, equally weighted',
              intro: 'What builder grade measures: recent GitHub output quality across the selected time window.',
              rows: [
              ...
              note: 'A = 80–100 · B = 60–79 · C = 40–59 · D = below 40. Trend arrow compares the current window to the previous matching window.',
            },
          ].map(block => (
            <div key={block.title} style={{
              ...
              <div style={{
                ...
                {block.rows.map(row => ([
                  <div key={`${block.title}-${row.label}-label`} style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.45 }}>
                    {row.label}
                  </div>,
                  <div
                    key={`${block.title}-${row.label}-weight`}
                    style={{
                      fontSize: '12px',
                      color: 'var(--text-primary)',
                      fontWeight: 500,
