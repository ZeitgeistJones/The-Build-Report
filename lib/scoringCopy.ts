...
export const RESCORE_PROMO_TOOLTIP =
  `Limited launch promo: on stale repos, earn ~$0.01 per stale commit to your wallet plus the same amount queued for CLAWD burns (50/50 treasury subsidy). Hold 10M $CLAWD to qualify. Accepted holder context can still be rescored anytime. Promo ends when disabled or the treasury runs low. ${APPROX_USD_NOTE}`
